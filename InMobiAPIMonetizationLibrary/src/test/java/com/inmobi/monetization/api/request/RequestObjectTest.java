...
public class RequestObjectTest {
	...
	public void testDevice() {
		...
		// androidId
		String androidId, gpid, ida, ua, ip;
		androidId = "  ";
		device.setAndroidId(androidId);
		assertNull(device.getAndroidId());
		androidId = "android";
		device.setAndroidId(androidId);
		assertEquals(androidId, device.getAndroidId());

		// gpid
		gpid = "  ";
		device.setGpId(gpid);
		assertNull(device.getGpId());
		gpid = "gpid";
		device.setGpId(gpid);
		assertEquals(gpid, device.getGpId());

		// ida
		ida = "  ";
		device.setIda(ida);
		assertNull(device.getIda());
		ida = "ida";
		device.setIda(ida);
		assertEquals(ida, device.getIda());
	...
	public void testRequest() {
		...
		assertFalse(request.isValid());
		request.setDevice(new Device("ca", "ua"));
